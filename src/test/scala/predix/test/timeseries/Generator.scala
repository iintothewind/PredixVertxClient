...
object Generator {
  def genDataPts(maxMeasurement: Double): Stream[JsonArray] = {
    ...
    Stream.iterate(new JsonArray().add(currentTime).add(Random.nextInt(maxMeasurement.toInt).toDouble + Random.nextDouble()).add(Random.nextInt(4)))(ja => new JsonArray().add(ja.getLong(0) + 1000).add(Random.nextInt(maxMeasurement.toInt).toDouble + Random.nextDouble()).add(Random.nextInt(4)))
  }
  ...
  def genMessage(tag: String, numOfDpts: Int, maxMeasurement: Double, attributeName: String, maxAttributeValue: Int): JsonObject = {
    ...
    new JsonObject()
      ...
      .put("body", new JsonArray().add(new JsonObject()
        .put("name", tag)
        .put("datapoints", genDataPts(maxMeasurement).take(numOfDpts).toList.foldRight(new JsonArray())((point, array) => array.add(point)))
