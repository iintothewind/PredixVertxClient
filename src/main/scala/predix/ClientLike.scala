...
trait ClientLike {
  ...
  implicit lazy val cfg: Config = Try(
    ConfigFactory.load("vertx.conf"))
    .getOrElse(
      ConfigFactory.empty()
  ...
  implicit lazy val proxyOptions: Option[ProxyOptions] = Try(
    ProxyOptions()
      .setType(ProxyType.HTTP)
      .setHost(cfg.getString("vertx.http.proxy.host"))
      .setPort(cfg.getInt("vertx.http.proxy.port")))
    .toOption
  implicit lazy val webClientOptions: WebClientOptions = proxyOptions match {
    case None =>
      WebClientOptions()
        .setSsl(cfg.getBoolean("vertx.http.ssl"))
        .setTrustAll(cfg.getBoolean("vertx.http.trustAll"))
        .setFollowRedirects(cfg.getBoolean("vertx.http.followRedirects"))
        .setTryUseCompression(cfg.getBoolean("vertx.http.tryUseCompression"))
        .setMaxPoolSize(cfg.getInt("vertx.http.maxPoolSize"))
    case Some(proxy) =>
      WebClientOptions()
        .setSsl(cfg.getBoolean("vertx.http.ssl"))
        .setTrustAll(cfg.getBoolean("vertx.http.trustAll"))
